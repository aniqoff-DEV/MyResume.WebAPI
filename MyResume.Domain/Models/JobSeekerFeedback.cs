...
{
    public class JobSeekerFeedback
    {
        ...
        private JobSeekerFeedback(Guid id, Guid jobSeekerId, string title, string description, int numberStars)
        {
            Id = id;
        ...
        public static Result<JobSeekerFeedback> Create(
            Guid id,
            Guid jobSeekerId,
            string title,
            string description,
            int numberStars)
        {
            if (title.Length > MAX_LENGHT_TITLE || title.Length < MIN_LENGHT_TITLE)
                return Result.Failure<JobSeekerFeedback>
            ...
            if (!isValidAssessment(numberStars))
                return Result.Failure<JobSeekerFeedback>("Error: the score does not correspond to a number from 1 to 5");

            var feedBack = new JobSeekerFeedback(id, jobSeekerId, title, description, numberStars);
